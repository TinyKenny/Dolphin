...
class GUI:
    ...
    def read_input(self):  #does not work work woth server
        ...
        if self.user_input.get()[0:1] == "/":
            if self.command_interpreter(self.user_input.get()):  # lauches command intepreter
                s.close()
                os._exit(0)
        else:
            #s.send(str.encode(self.user_input.get()))  # was not a command
            self.print_to_log((self.username +  ":" + self.user_input.get()))
        self.user_input.set("")
    ...
    def manual_connect(self):
        ...
        chat_server.set("127.0.0.1")
        chat_server_field=Entry(connet_window,
                              width=16,
                              exportselection=0,
                              textvariable=chat_server,
                              bg="white")
        ...
        ipv4_butt = Radiobutton(network_protocol_frame, text="IPv4", variable=network_protocol, value="IPv4")
        ipv4_butt.grid()
        ipv4_butt.select()

        ipv6_butt = Radiobutton(network_protocol_frame, text="IPv6", variable=network_protocol, value="IPv6")
        ipv6_butt.grid()
    ...
    def connect(self):
        ...
        try:
            self.s.connect((chat_server, port))
            self.print_to_log(("Connected to:", chat_server + ":" + str(port)))
            ...
            self.print_to_log(("Cound not connect to", chat_server + ":" + str(port) + "\n" + str(e)))
